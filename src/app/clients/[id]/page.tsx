...
export default function ClientDetails({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    const fetchClientData = async () => {
      try {
        ...
        const dealsRes = await fetch(`/api/pipeline/cards?clientId=${params.id}`);
        if (dealsRes.ok) {
  ...
  const handleClientDelete = async () => {
    if (!client || !window.confirm(`Are you sure you want to delete "${client.name}"? This will also delete all associated deals.`)) {
      return;
    }
  ...
  if (!client) {
    return (
      <Container maxWidth="lg" sx={{ py: 4 }}>
        ...
        <Button component={Link} href="/pipeline" sx={{ mt: 2 }}>
          Back to Pipeline
        </Button>
      </Container>
    );
          ...
          <Paper sx={{ p: 3 }}>
            ...
            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
              ...
              <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                ...
                <Typography variant="body2" sx={{ color: theme.palette.text.secondary }}>
                  Total Value: {formatCurrency(client.valueUsd)}
                </Typography>
              </Box>
                    ...
                    <CardContent sx={{ p: 2 }}>
                      ...
                      <Box sx={{ display: 'flex', gap: 1, alignItems: 'center' }}>
                        ...
                        <Chip
                          ...
                          variant="outlined"
                          sx={{
                            borderColor: getPriorityColor(deal.priority, theme),
                            color: getPriorityColor(deal.priority, theme),
                          }}
                        />
